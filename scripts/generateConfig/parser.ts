...
export const getFarms = (data) => {
  const farms: FarmConfig = data.map((farm) => {
    return {
      ...
      lpAddresses: {
        106: farm.lp_mainnet_address,
        111: '0xE66790075ad839978fEBa15D4d8bB2b415556a1D',
      },
      tokenSymbol: farm?.token?.symbol,
      tokenAddresses: {
        106: farm?.token?.mainnet_address,
